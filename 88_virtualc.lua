...
local defaultProxyMetatable = {
 __index = function(self, key)
  if key == "slot" then return self:getSlot() end -- dynamic slot
  return rawget(self, key)
 end
}

local defaultGetMethodInfo = function() return true, "undocumented" end 
local defaultGetFields = function() return {} end
local defaultGetSlot = function() return -1 end
local defaultCreateProxy = function(self)
 local proxy = setmetatable({}, defaultProxyMetatable)
 ...
 for _, method in pairs(self:getMethods(address)) do
...
local function mountComponent(address, descriptor)
 ...
 assert(checkType(descriptor.type, "string"),                   "invalid descriptor: type must be a string")
 assert(checkType(descriptor.createProxy, "function", "nil"),   "invalid descriptor: createProxy must be a function or nil")
 assert(checkType(descriptor.invokeMethod, "function"),         "invalid descriptor: invokeMethod must be a function")
 assert(checkType(descriptor.getMethodInfo, "function", "nil"), "invalid descriptor: getMethodInfo must be a function or nil")
 assert(checkType(descriptor.getMethods, "function"),           "invalid descriptor: getMethods must be a function")
 assert(checkType(descriptor.getFields, "function", "nil"),     "invalid descriptor: getFields must be a function or nil")
 assert(checkType(descriptor.getSlot, "function", "nil"),       "invalid descriptor: getSlot must be a function or nil")
...
do -- intercept the component library
 ...
 function component.invoke(address, method, ...)
  local virtual = virtualComponents[address]
  if virtual then
   return virtual:invokeMethod(method, ...)  
  end
